Public Class ClassFGTrf
    ...
    Public Sub changeStatus(ByVal id_report_par As String, ByVal id_status_reportx_par As String, ByVal type_restock As String)
        ...
        If id_status_reportx_par = "6" Then
            ...
            ElseIf type_restock = "2" Then
                ...
                id_oos = data_oos.Rows(0)("id_ol_store_oos").ToString
                id_comp_group = data_oos.Rows(0)("id_comp_group").ToString
                id_api_type = data_oos.Rows(0)("id_api_type").ToString
                id_web_order = data_oos.Rows(0)("id_order").ToString
                ...
                'get id drawer group
                Dim query_drawer_grp As String = "SELECT wh_normal.id_drawer_def AS `id_drawer_normal`,
                wh_sale.id_drawer_def AS `id_drawer_sale`
                ...
                Dim data_drawer_grp As DataTable = execute_query(query_drawer_grp, -1, True, "", "", "", "")
                Dim id_wh_type_induk As String = execute_query("SELECT c.id_wh_type 
                FROM tb_fg_trf t
                INNER JOIN tb_m_comp_contact cc ON cc.id_comp_contact = t.id_comp_contact_to
                INNER JOIN tb_m_comp c ON c.id_comp = cc.id_comp
                WHERE t.id_fg_trf='" + id_report_par + "'", 0, True, "", "", "", "")
                Dim id_used_drawer As String = ""
                If id_wh_type_induk = "1" Then
                    'normal
                    id_used_drawer = data_drawer_grp.Rows(0)("id_drawer_normal").ToString
                Else
                    'sale
                    id_used_drawer = data_drawer_grp.Rows(0)("id_drawer_sale").ToString
                End If
                ...
                DELETE FROM tb_storage_fg 
                WHERE report_mark_type=57 AND id_report=" + id_report_par + ";
                -- delete reserved restock oos
                DELETE FROM tb_storage_fg WHERE report_mark_type=278 AND id_report=" + id_oos + " AND storage_product_notes='restock' AND report_mark_type_ref='57' AND id_report_ref='" + id_report_par + "' ;
                -- insert storage
                INSERT INTO tb_storage_fg(id_wh_drawer, id_storage_category, id_product, bom_unit_price, report_mark_type, id_report, storage_product_qty, storage_product_datetime, storage_product_notes, id_stock_status, report_mark_type_ref, id_report_ref) "
                query_complete += "SELECT (getCompByContact(trf.id_comp_contact_from, 4)) AS `drawer`, '1', trf_det.id_product, dsg.design_cop, '39' AS `report_mark_type`, trf.id_sales_order AS `id_report`, trf_det.fg_trf_det_qty, NOW(), '', '2', 57, " + id_report_par + " "
                query_complete += "FROM tb_fg_trf trf "
                ...
                query_complete += "WHERE trf.id_fg_trf=" + id_report_par + " AND trf_det.fg_trf_det_qty >0  "
                query_complete += "UNION ALL 
                SELECT (trf.id_wh_drawer) AS `drawer`, '2', trf_det.id_product, dsg.design_cop, '57' AS `report_mark_type`, trf.id_fg_trf AS `id_report`, trf_det.fg_trf_det_qty, NOW(), '', '1', NULL, NULL "
                query_complete += "FROM tb_fg_trf trf "
                query_complete += "INNER JOIN tb_fg_trf_det trf_det ON trf_det.id_fg_trf = trf.id_fg_trf "
                query_complete += "INNER JOIN tb_m_product prod ON prod.id_product = trf_det.id_product "
                query_complete += "INNER JOIN tb_m_design dsg ON dsg.id_design = prod.id_design "
                query_complete += "WHERE trf.id_fg_trf=" + id_report_par + " AND trf_det.fg_trf_det_qty >0
                UNION ALL
                SELECT '" + id_used_drawer + "' AS `drawer`, '1', trf_det.id_product, IFNULL(dsg.design_cop,0), '278' AS `report_mark_type`, '" + id_oos + "' AS `id_report`, trf_det.fg_trf_det_qty, NOW(), 'restock', '1', '57', trf.id_fg_trf 
                FROM tb_fg_trf trf 
                ...
                UNION ALL
                SELECT '" + id_used_drawer + "' AS `drawer`, '2', trf_det.id_product, IFNULL(dsg.design_cop,0), '278' AS `report_mark_type`, '" + id_oos + "' AS `id_report`, trf_det.fg_trf_det_qty, NOW(), 'restock', '2', '57', trf.id_fg_trf 
                FROM tb_fg_trf trf 
                INNER JOIN tb_fg_trf_det trf_det ON trf_det.id_fg_trf = trf.id_fg_trf 
                INNER JOIN tb_m_product prod ON prod.id_product = trf_det.id_product 
                INNER JOIN tb_m_design dsg ON dsg.id_design = prod.id_design 
                WHERE trf.id_fg_trf=" + id_report_par + " AND trf_det.fg_trf_det_qty>0;
                CALL create_oos_restock_alloc_grp(" + id_web_order + ", " + id_comp_group + ", " + id_product_restock + ", " + qty_restock + ");
                CALL set_log_web_order_grp(" + id_web_order + ", CONCAT('Completed stock for restock')," + id_comp_group + ");"
                execute_non_query_long(query_complete, True, "", "", "", "")
            End If
        ...
        Try
            ...
            End If
        Catch ex As Exception
            stopCustom("Problem execute. Don't continue process. Please contact administrator. Detail: id_trf=" + id_report_par + ";id_status=" + id_status_reportx_par + ";type=" + type_restock + ";" + ex.ToString)
        End Try
