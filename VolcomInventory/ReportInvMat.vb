Public Class ReportInvMat
    ...
    Private Sub ReportInvMat_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles MyBase.BeforePrint
        Dim query_head As String = "SELECT inv.ref_date,inv.`id_inv_mat`,SUM(invd.`value`) AS total,CAST(inv.vat_percent AS DECIMAL(5,0)) AS p,SUM(invd.`value`)*(inv.vat_percent/100) AS ppn,SUM(invd.`value`)*((100+inv.vat_percent)/100) AS total_amount
