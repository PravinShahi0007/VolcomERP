Public Class FormStockTakeStorePeriodDet
    ...
    Private Sub SBSave_Click(sender As Object, e As EventArgs) Handles SBSave.Click
        ...
        For x = 0 To id_store.Count - 1
            ...
            Dim j_m_product_store As String = tableToJson("tb_m_product_store", "SELECT id_product, id_design, full_code, `code`, `name`, size, class, color, color_desc, sht, unit_cost, is_old_design FROM tb_m_product_store")
            ...
            Dim j_m_user As String = tableToJson("tb_m_user", "SELECT id_user, id_role, id_employee, id_store, username, `password`, 2 AS is_change, name_external, position_external, email_external, is_external_user FROM tb_m_user WHERE id_user IN (SELECT value_id FROM tb_opt_include_st_store WHERE table_name = 'tb_m_user') UNION ALL SELECT id_user, id_role, id_employee, id_store, username, `password`, is_change, name_external, position_external, email_external, is_external_user FROM tb_m_user WHERE id_user IN (SELECT id_user FROM tb_m_user WHERE id_store = (SELECT id_store FROM tb_st_store_period WHERE id_st_store_period = " + id_st_store_period + "))")
