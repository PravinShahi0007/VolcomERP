...
WHERE po.id_report_status='6' AND po.is_close_rec='2'"
    ...
    Sub load_req()
        ...
        '
        Dim query As String = "SELECT '-' AS status_val,cat.id_expense_type,CONCAT(rd.item_detail,'\r\n',IFNULL(rd.remark,'')) AS item_detail,rd.id_b_expense,rd.id_b_expense_opex,icd.id_vendor_type,icd.item_cat_detail,vt.vendor_type,req.date_created AS pr_created,dep.`departement`,rd.`id_purc_req_det`,req.`id_purc_req`,req.`purc_req_number`,cat.`item_cat`,itm.`item_desc`,rd.`value` AS val_pr,rd.`qty` AS qty_pr,'no' AS is_check 
...
,pod.`id_purc_order_det`,po.`purc_order_number`,c.`comp_number`,c.`comp_name`,it.id_item,it.`item_desc`,CONCAT(reqd.item_detail,'\r\n',IFNULL(reqd.remark,'')) AS item_detail,pod.`value`,(pod.`qty`*pod.`value`) AS tot_value
FROM tb_purc_order_det pod
