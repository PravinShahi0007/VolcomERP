Public Class FormPolis
    ...
    Sub load_polis_toko(ByVal opt As String)
        ...
        If opt = "All" Then
            q = "SELECT p.id_polis,DATEDIFF(p.end_date,DATE(NOW())) AS expired_in,pol_by.comp_name AS comp_name_polis,CONCAT(c.comp_number,' - ',c.`comp_name`) AS polis_object,c.`address_primary` AS polis_object_location,p.`number` AS polis_number,d.`description` AS polis_untuk,p.`premi`,p.`start_date`,p.`end_date`,p.nilai_total
FROM tb_polis p 
INNER JOIN tb_m_comp c ON c.`id_comp`=p.`id_reff` AND p.`id_polis_cat`=1 AND c.is_active=1
