Public Class FormReportMark
    ...
    Private Sub view_report_status(ByVal lookup As DevExpress.XtraEditors.LookUpEdit)
        ...
        ElseIf report_mark_type = "416" Then
            'close gwp pos
            query = String.Format("SELECT id_report_status, report_number FROM tb_close_promo_rules WHERE id_close_promo_rules = '{0}'", id_report)
        End If
...
WHERE a.id_adj_in_fg = '" & id_report & "'"
        ...
        ElseIf report_mark_type = "148" Then
            ...
            Dim id_report_now As String = execute_query("SELECT id_report_status FROM tb_purc_rec WHERE id_purc_rec='" & id_report & "'", 0, True, "", "", "", "")
            ...
            If id_report_now = "6" And id_status_reportx = "5" Then
                'jika sudah complete mau di cancel
                Dim qjd As String = ""
                ...
                SELECT IF(rq.is_store_purchase=1,'" & id_purc_store & "',rq.id_departement) AS id_departement, 2, rd.id_item, (pod.`value`/i.stock_convertion) AS value, 148, " + id_report + ", rd.qty_stock, NOW(),'', 1
                ...
                'det journal balik
                qjd = "INSERT INTO tb_a_acc_trans_det(id_acc_trans, id_acc, id_vendor, id_comp, qty, credit, debit,  acc_trans_det_note, report_mark_type, id_report, report_number, report_mark_type_ref, id_report_ref, report_number_ref, id_coa_tag)
SELECT '" & id_acc_trans & "' AS id_acc_trans,id_acc,id_vendor,id_comp,qty,debit,credit,CONCAT('Cancel Form, ',acc_trans_det_note) AS note,report_mark_type,id_report,report_number,report_mark_type_ref,id_report_ref,report_number_ref,id_coa_tag
FROM
`tb_a_acc_trans_det`
WHERE report_mark_type='148' AND id_report='" & id_report & "'"
                execute_non_query(qjd, True, "", "", "", "")
            Else
                'completed
                Dim qjd As String = ""
...
WHERE pocd.id_prod_order_close = '" & id_report & "'"
        ...
        ElseIf report_mark_type = "221" Then
            ...
            'cari status di db
            Dim dt_report As DataTable = execute_query("SELECT dn.id_report_status AS id_report_status,SUM(dnd.qty*(dnd.claim_percent/100)*dnd.unit_price) AS val
...
INNER JOIN tb_range r ON r.id_range=s.id_range AND r.is_md=1
LEFT JOIN (
	SELECT dc.id_design, 
	MAX(CASE WHEN cd.id_code=32 THEN cd.id_code_detail END) AS `id_division`,
	MAX(CASE WHEN cd.id_code=32 THEN cd.code_detail_name END) AS `division`,
        ...
        ElseIf report_mark_type = "416" Then
            ...
            If id_status_reportx = "3" Then
                ...
                Try
                    ...
                    c.syncCloseGWP(id_report)
                Catch ex As Exception
                    sync_status = "2"
                    sync_message = ex.ToString
                End Try

                execute_non_query("INSERT INTO tb_pos_sync (sync_type, sync_status, message, created_at) VALUES ('Close GWP: Close GWP POS (416)', " + sync_status + ", '" + addSlashes(sync_message) + "', NOW())", True, "", "", "", "")
            End If

            query = String.Format("UPDATE tb_close_promo_rules SET id_report_status = '{0}' WHERE id_close_promo_rules = '{1}'", id_status_reportx, id_report)
