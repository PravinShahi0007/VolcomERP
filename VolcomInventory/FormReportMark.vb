        ...
        ElseIf report_mark_type = "157" Then
            ...
            If id_status_reportx = "6" Then
                ...
                Else
                    ...
                    WHERE e.id_item_expense=" + id_report + " AND e.vat_total>0
                    UNION ALL
                    SELECT " + id_acc_trans + ", e.id_acc_from, 0 AS `debit`, e.`total` AS `credit`, '' AS description, 157, e.id_item_expense, e.`number`,1
                    FROM tb_item_expense e
                    WHERE e.id_item_expense=" + id_report + " "
                    execute_non_query(qjd, True, "", "", "", "")
                End If
            ...
            If id_status_reportx = "6" Then
                ...
                Dim qjd As String = "INSERT INTO tb_a_acc_trans_det(id_acc_trans, id_acc, id_comp, qty, debit, credit, id_currency, kurs, debit_valas, credit_valas, acc_trans_det_note, report_mark_type, id_report, report_number,report_mark_type_ref, id_report_ref, report_number_ref, vendor)
                                    ...
                                    (
                                        ...
                                        UNION ALL
	                                    /* Hutang dagang */
                                        SELECT '" & id_acc_trans & "' AS id_acc_trans,pnd.id_acc AS `id_acc`, pnd.id_comp,  0 AS `qty`,IF(pnd.id_dc=2,IF(pnd.value<0,ABS(pnd.value),0),IF(pnd.value<0,0,ABS(pnd.value))) AS `debit`, IF(pnd.id_dc=2,IF(pnd.value<0,0,ABS(pnd.value)),IF(pnd.value<0,ABS(pnd.value),0)) AS `credit`, pnd.id_currency, pnd.kurs, IF(pnd.id_dc=2,0,ABS(pnd.val_bef_kurs)) AS debit_valas, IF(pnd.id_dc=2,ABS(pnd.val_bef_kurs),0) AS credit_valas,'' AS `note`,159 AS report_mark_type,pn.id_pn AS id_report, pn.number AS report_number,pnd.report_mark_type,pnd.id_report,pnd.number,pnd.vendor
	                                    FROM tb_pn_det pnd
