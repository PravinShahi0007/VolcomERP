...
INNER JOIN tb_m_comp c ON c.`id_comp`=cc.`id_comp` AND c.id_comp='" & FormInvoiceFGPODP.SLEVendor.EditValue.ToString & "'
LEFT JOIN (
	SELECT pod.id_sample_purc,pod.`id_sample_purc_det`,SUM(recd.sample_purc_rec_det_qty*recd.fob_price_update) AS amount_rec,SUM(recd.sample_purc_rec_det_qty) AS qty_rec,IFNULL(SUM(cls.qty_close),0) AS qty_close
    ...
    )cls ON cls.id_sample_purc_det=pod.`id_sample_purc_det`
