Public Class FormEmpPayrollDeduction
    ...
    Sub load_deduction()
        Dim query As String = "SELECT pyd.id_payroll_deduction,emp.`id_employee`,dep.`departement`,emp.`employee_code`,emp.`employee_name`,emp.`employee_position`,lvl.`employee_level`,pyd.`deduction`,sald.`salary_deduction`,saldc.salary_deduction_cat,pyd.note FROM tb_emp_payroll_deduction pyd
                                ...
                                ORDER BY emp.`id_employee_level` ASC, emp.`employee_code` ASC, sald.`id_salary_deduction` ASC"
