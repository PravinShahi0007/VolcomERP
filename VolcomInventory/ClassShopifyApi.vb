        ...
        Using dataStream As IO.Stream = response.GetResponseStream()
            ...
            If json("orders").Count > 0 Then
                For Each row In json("orders").ToList
                    ...
                    shipping_city, shipping_post_code, shipping_region, payment_method, tracking_code, ol_store_sku, ol_store_id, sku, sales_order_det_qty) VALUES "
                    ...
                    For Each row_item In row("line_items").ToList
                        ol_store_sku = row_item("variant_id").ToString
                        ol_store_id = row_item("id").ToString
                        sku = row_item("sku").ToString
                        sales_order_det_qty = row_item("quantity").ToString

                        If i > 0 Then
                            qins += ","
                        End If
                        qins += "('" + id + "', '" + sales_order_ol_shop_number + "', '" + sales_order_ol_shop_date + "', '" + customer_name + "', '" + shipping_name + "', '" + shipping_address + "', '" + shipping_phone + "', 
                        '" + shipping_city + "', '" + shipping_post_code + "', '" + shipping_region + "', '" + payment_method + "', '" + tracking_code + "', '" + ol_store_sku + "', '" + ol_store_id + "', '" + sku + "', '" + sales_order_det_qty + "') "
                        i += 1
                    Next
