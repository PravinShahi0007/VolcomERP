    ...
    Private Sub ReportSalesReturn_BeforePrint(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles MyBase.BeforePrint
        ...
        Dim query As String = "
            ...
            FROM (
                ...
                WHERE a.report_mark_type='" & rmt & "' AND a.id_report='" & id_sales_return_qc & "' " + If(id_pre = "-1", "AND a.is_use = 1 AND a.id_mark = 2", "AND (a.level IS NULL OR a.level = 1)") + "
                ORDER BY a.id_report_status, a.id_mark_asg)
                UNION
                (SELECT 5 AS `order`, 'Completed By,' AS report_status_display, employee_name, employee_position AS role, (SELECT DATE_FORMAT(final_comment_date, '%d-%m-%Y %H:%i') FROM tb_report_mark_final_comment WHERE report_mark_type = " & rmt & " AND id_report = " + id_sales_return_qc + " LIMIT 1) AS date_time
                FROM tb_m_employee
