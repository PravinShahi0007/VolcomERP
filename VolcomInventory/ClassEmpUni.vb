Public Class ClassEmpUni
    ...
    Public Function queryMainOrder(ByVal condition As String, ByVal order_type As String) As String
        If order_type = "1" Then
            order_type = "ASC "
        ElseIf order_type = "2" Then
            order_type = "DESC "
        ...
        Dim query As String = "SELECT so.id_sales_order, so.sales_order_number, so.sales_order_date, so.sales_order_note, SUM(IFNULL(sod.sales_order_det_qty,0)) AS `total_order`, (IFNULL(sod.design_price,0)*IFNULL(sod.sales_order_det_qty,0)) AS `amount`,
        so.id_emp_uni_budget, b.id_emp_uni_period, p.period_name, b.budget, bu.budget_use, (b.budget-bu.budget_use) AS `budget_rmg`, so.tolerance, so.discount,
        ...
        LEFT JOIN(
            SELECT so.id_sales_order, so.id_emp_uni_budget,SUM(IFNULL(sod.design_price,0) * IFNULL(sod.sales_order_det_qty,0)) AS `budget_use` 
            FROM tb_sales_order so 
            LEFT JOIN tb_sales_order_det sod ON sod.id_sales_order = so.id_sales_order
            WHERE !ISNULL(so.id_emp_uni_budget) AND so.id_sales_order!=5
