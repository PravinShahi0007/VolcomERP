Public Class FormPurcPayment
    ...
    Sub view_receive()
        ...
        Dim query As String = "SELECT 'no' AS is_check,pr.`id_purc_rec`,pr.`date_created`,pr.`purc_rec_number`,pr.`date_arrived`,pr.`do_vendor_number`,po.purc_order_number,c.`comp_name`,c.`comp_number`,SUM(prd.`qty`) AS qty_rec,SUM(prd.`qty`*pod.`value`) AS val_rec FROM tb_purc_rec_det prd
