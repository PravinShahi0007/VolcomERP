Public Class FormStockTakePropose
    ...
    Sub load_form()
        Dim query As String = "
            SELECT p.id_st_store_propose, p.number, DATE_FORMAT(p.start_period, '%d %M %Y') AS start_period, DATE_FORMAT(p.end_period, '%d %M %Y') AS end_period, CONCAT(g.comp_group, ' - ', g.description) AS comp_group, DATE_FORMAT(p.created_at, '%d %M %Y %H:%i:%s') AS created_at, e.employee_name AS created_by
