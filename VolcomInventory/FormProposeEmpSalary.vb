Public Class FormProposeEmpSalary
    ...
    Sub load_pps()
        Dim query As String = "
            SELECT sal.id_employee_sal_pps, sal.number, cat.sal_pps_category, typ.sal_pps_type, DATE_FORMAT(sal.effective_date, '%d %M %Y') AS effective_date, sal.note, IFNULL(sts.report_status, 'Draft') AS report_status, emp.employee_name AS created_by, DATE_FORMAT(sal.created_at, '%d %M %Y %H:%i:%s') AS created_at
            ...
            LEFT JOIN tb_lookup_employee_sal_pps_category AS cat ON sal.id_sal_pps_category = cat.id_sal_pps_category
