...
WHERE `id_design`='" & id_design & "' "
    ...
    Private Sub BApprove_Click(sender As Object, e As EventArgs) Handles BApprove.Click
        ...
        Else
            ...
            Else
                'final
                Dim query As String = "UPDATE tb_m_design SET id_cop_status=2,pp_is_approve='1',pp_is_approve_date=NOW(),pp_approve_by='" & id_user & "',final_is_approve='1',final_is_approve_date=NOW(),final_approve_by='" & id_user & "',design_cop='" & decimalSQL((TEUnitPrice.EditValue + TEAddCost.EditValue).ToString) & "',design_cop_addcost='" & decimalSQL(TEAddCost.EditValue.ToString) & "' WHERE id_design='" & id_design & "'"
                execute_non_query(query, True, "", "", "", "")
                '
                Try
                    Dim nm As New ClassSendEmail
